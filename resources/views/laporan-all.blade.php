...
<body>
    ...
    {{-- Body --}}
    <div class="container ">
        <label class="text-header">Nomor Form : {{$head->penilaian_id ?? ''}}</label><br>
        <label class="mb-3 text-header">Nama Form : {{$head->penilaian->penilaian_name ?? ''}}</label><br>
        <label class="mb-3 text-header">Jadwal : {{$head->penilaian->jadwal->start_date->format('d F Y') ?? ''}}</label>
        <table>
            <tbody>
                <tr>
                    <th>No</th>
                ...
                <tr>
                    ...
                    <td>{{$p->user->name ?? ''}}</td>
                    <td style="text-align: center;">{{$p->nilai}} / {{$p->jumlah}}</td>
                    <td style="text-align: center;">{{Round(($p->nilai / $p->jumlah) * 100)}}</td>
                </tr>
                @endforeach
